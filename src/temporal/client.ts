...
/**
 * Temporal client for starting Logan remediation pipeline workflows.
 *
 ...
 *   --workflow-id <id>    Custom workflow ID (default: logan-<timestamp>)
...
function showUsage(): void {
  console.log('\nLogan Temporal Client');
  console.log('Start a remediation pipeline workflow\n');
  console.log('Usage:');
  console.log(
    '  node dist/temporal/client.js <webUrl> <repoPath> [options]\n'
  );
...
async function resolveWorkspace(
  ...
  if (!args.resumeFromWorkspace) {
    ...
    const workflowId = args.customWorkflowId || `${hostname}_logan-${Date.now()}`;
    return {
      workflowId,
  ...
  return {
    workflowId: `${workspace}_logan-${Date.now()}`,
    sessionId: workspace,
    isResume: false,
    terminatedWorkflows: [],
  };
...
function displayMonitoringInfo(args: CliArgs, workspace: WorkspaceResolution): void {
  ...
  console.log(`  Web UI:  http://localhost:8234/namespaces/default/workflows/${workspace.workflowId}`);
  console.log(`  Logs:    ./logan logs ID=${workspace.workflowId}`);
  console.log();
  console.log('Output:');
...
async function startPipeline(): Promise<void> {
  ...
  try {
    ...
    const handle = await client.workflow.start<(input: PipelineInput) => Promise<PipelineState>>(
      'remediationPipelineWorkflow',
      {
        taskQueue: 'logan-pipeline',
        workflowId: workspace.workflowId,
        args: [input],
      }
    );
