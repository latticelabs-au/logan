...
/**
 * Workspace listing tool for Logan.
 *
...
async function listWorkspaces(): Promise<void> {
  ...
  if (workspaces.length === 0) {
    console.log('\nNo workspaces found.');
    console.log('Run a pipeline first: ./logan start URL=<url> REPO=<repo>');
    return;
  }
  ...
  if (resumableCount > 0) {
    console.log('\nResume with: ./logan start URL=<url> REPO=<repo> WORKSPACE=<name>');
  }
